...
更新时间:2026-05-06
基线分支:`main`  
基线提交:`4d42b06 docs(handoff): record llm variance smoke (#188)`
基线 CI:`25411582418` 全绿(`lint-and-typecheck` / `test` / `e2e` / `build`; `prompt-regression` 全绿; `docker` 按条件跳过)
...
- 2026-05-06 维护批次已闭合:#182 registry `computedAt` 归一、#183 admin position/recommendation shared types、#184 shared prebuild guards、#185 e2e lint CI、#186 candidate guard server status、#187 e2e helper lint zero-warning、#188 LLM variance skip-safe smoke 记录。

## 已知未修复 / V5.1 待做

### P1 · 真实 LLM provider 方差监控

状态:A14b deterministic 主线 gate 已闭合;真实 provider nightly/manual workflow 已有 skip-safe 执行面。V5.1 prep 已补结构化诊断:脚本能区分 prompt drift、provider failure、provider variance、parser/schema drift、live-path drift。2026-05-06 手动触发 `LLM Variance` run `25411527017` 已通过 skip-safe plumbing:无 GLM/Claude secret 时成功退出并输出明确 annotation。
入口:
...
状态:`ExamGuard` / `ProfileGuard` 已改为读取 server-side candidate session status;localStorage consent/profile flag 只作为服务端状态确认后的缓存。
入口:
...
- 后续如需更细入口控制,可在 status payload 增加 self-view eligibility 或 exam availability。
- 保持 guard fail-closed:status 读取失败或 token 无效时不放行。
...
状态:e2e TypeScript 已纳入 CI;e2e ESLint 也已纳入 `lint-and-typecheck`;helper lint warnings 已清零。e2e job 已加 Playwright browser cache,避免 docs-only PR 反复长时间下载 Chromium。
入口:
...
- 保持 `npm run lint:e2e` 0 error / 0 warning。
...
状态:root 已有 `build:shared`;client/server direct `dev` / `build` / `test` 前会自动构建 shared,避免读取陈旧 `dist`。
入口:
...
- 若后续引入 TS project references,再评估是否替换 lifecycle guard。
- 保持 root `build --workspaces` 与 direct workspace 命令都能独立成功。
...
状态:`CursorBehaviorLabel` 后端计算已进入 orchestrator;低样本 / 非 MB suite 仍返回 undefined,避免伪造画像。
入口:`packages/server/src/services/scoring-orchestrator.service.ts`。
建议交付:

- 观察真实 MB 行为数据后校准阈值,尤其是 accept-rate / chat-direct ratio / latency 的边界。
- 若要增加 `CursorBehaviorLabel.score` / confidence,先扩 shared shape + report explanation,不要只加服务端字段。
- Report view 已显示标签和 evidence signal count;若进入更强产品叙事,补每个 evidence signal 的可读解释。
...
- cross-repo mock sync rule 已写入 `claude-self-check-checklist-v2.md` 规则 12;后续执行时保持四面检查(shared / server / client adapter / client mock)。
...
2. 做真实 provider LLM variance:配置 GLM/Claude secret 后复用 A14b tolerance-band,先 manual/nightly,稳定后再考虑进主 CI。
3. 做 generator prompt productionization:仅在 generator pipeline 真启动时补 step0-step8 v2 prompt + regression,fail-closed 保持。
4. 做 socket deprecation observability:量化 payload fallback / V4 self-assess bridge usage,再决定删除窗口;没有 0-usage 证据前不删除 bridge。
5. 做 scoring replay policy 产品化:当前 snapshot stamp 已写入;只有需要 admin 可见版本、显式 refresh/backfill 或 legacy rename 时再动代码。
6. 做 report/candidate boundary audit:继续防止 full report 泄露到 candidate surface,把新增 candidate 页面纳入同一断言。
7. 做 P3 增强:CursorBehaviorLabel 阈值校准 / capability profile evidence 若进入 V5.1,必须同时做 shared shape、scoring、report explanation。
8. 做文档 / 流程整理:清理 superseded planning 段落,但保留 current snapshot 指针和 observations lineage。
