...
状态:A14b deterministic 主线 gate 已闭合;真实 provider nightly/manual workflow 已有 skip-safe 执行面。
入口:
...
- 加 usage observability:记录 payload fallback / V4 self-assess bridge 命中率。
...
状态:V5.0 已禁止候选人访问 full report CTA;完成页和 self-view 已有 DOM 级 `/report` / `/admin` 链接防回归断言,但 5.1 仍应继续做边界审计。
入口:
