...
更新时间:2026-05-05
基线分支:`main`  
基线提交:`85e7e9d feat(prompts): seed live V5 prompt versions (#156)`
基线 CI:`25366823262` 全绿(`lint-and-typecheck` / `test` / `e2e` / `build` / `docker`; `prompt-regression` 按条件跳过)
...
状态:A14b deterministic 主线 gate 已闭合;真实 provider/nightly gate 仍待做。
入口:`packages/server/src/__tests__/reliability/pure-rule-signals.test.ts` 中 `A14b · LLM signal variance monitoring`。
...
- 为 3 个 MD LLM whitelist signals 建立真实 provider nightly/manual workflow。
- 使用与 A14b deterministic gate 同一 tolerance-band 口径,不要改回 pure-rule deep-equal。
- CI secret 未准备时不要让主 CI flaky。
...
状态:9 个 live model-adjacent prompt 已有 active v2 seed;9 个 `generator.step0..8` future slot 仍是 placeholder/fail-closed。
入口:
...
状态:MC probe 的 replay 读源已收敛到 `Session.scoringResult.signals` canonical snapshot;长期 report snapshot policy 仍需要 5.1 决策。
入口:
