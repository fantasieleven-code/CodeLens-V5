...
| `behavior:batch`(`event.type=edit_session_completed`) | client → server | dispatched inside `behavior:batch` envelope; per-event `payload` shape: `{ filePath, startTime, endTime, keystrokeCount }` | Task 30a server-side dispatch in `behavior-handlers.ts` → `appendEditSessions` → `metadata.mb.editorBehavior.editSessions[]` → sEditPatternQuality | Task 30a 持久化 wired,**client emit 待 Task 30b**(目前由 Pattern H 7th gate `tests/integration/behavior-batch-pipeline.test.ts` 用 mock 事件 exercise,production 路径 dormant 至 30b 完成) |
