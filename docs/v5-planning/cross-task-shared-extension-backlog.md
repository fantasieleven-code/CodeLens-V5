...
- ~~**middleware envelope consistency**:`requireAdmin` / `requireOrg` / `requireOrgOwner` still emit flat `{ error: string }`. B-A12 auth-fallback patch only migrated `requireCandidate` to `next(AppError)` nested envelope(Frontend F-A12 + Consent path). Full 4-helper unification deferred — blocked by Frontend `AdminGuard`(PR #75)current flat-shape dependency.~~ **DONE 2026-04-30**:admin API client has `AdminApiError` nested/flat parser, and backend auth middleware now sends `requireAdmin` / `requireOrg` / `requireOrgOwner` failures through `next(AuthenticationError|AuthorizationError)` so global `errorHandler` emits the same nested AppError envelope as route handlers.
...
- ~~**admin error shape Frontend remap**:Pair with middleware envelope consistency above. Frontend admin surface needs a remap layer before backend can flip `requireAdmin` to nested envelope.~~ **DONE 2026-04-30**:`adminApi.ts` added `AdminApiError`, preserves nested AppError `{ code, message, details? }`, maps legacy flat 401/403 guard envelopes to `AUTH_REQUIRED`/`FORBIDDEN`, and keeps report-specific `REPORT_NOT_COMPLETED` UX behavior unchanged.
- ~~**Candidate.token audit**:historical HR-mint long-lived token on `Candidate` model(not Session-scoped)has unclear current consumers. Audit and decide keep / deprecate post-V5.0.5 once auth-fallback path is stable.~~ **DONE 2026-04-30**:repo audit found no active reader/writer for `Candidate.token` or `Candidate.tokenExpiresAt`; candidate exam auth uses `Session.candidateToken`/sessionId and self-view uses `Session.candidateSelfViewToken`. Prisma schema + migration now drop the person-scoped token columns to remove the cross-session replay footgun.
...
- ~~**admin session-lifecycle mint-list API**(Pattern D cleanup flag):admin `POST /admin/sessions/create` now mints 2 opaque Session-scoped tokens(`candidateToken` + `candidateSelfViewToken`).Considered a future `GET /admin/sessions/:id/links` endpoint that returns `{ candidateExamLink, candidateSelfViewUrl }` so admin UI can re-fetch after session creation without re-hitting mint(idempotent read).V5.0.5 · blocked by admin UI need(not urgent — current response envelope sufficient).~~ **DONE 2026-04-30**:`GET /admin/sessions/:sessionId/links` now returns the already-minted `candidateToken`, `candidateSelfViewToken`, `shareableLink`, and `selfViewUrl`; it never re-mints tokens and fails closed with 409 when legacy/corrupted sessions lack either token. Shared type `V5AdminSessionLinksResponse` and client `adminApi.getSessionLinks()` are wired.
...
- ~~**admin.ts Pattern D migration**(Pattern D cleanup flag):`admin.ts` 中 scoringResult handling 仍用 `as V5ScoringResult` cast(L374)、B-A10-lite self-view 路由已迁移到 `V5ScoringResultSchema.parse()` + cast-back pattern.Admin endpoint 4 `getAdminSessionReport` 可在 V5.0.5 内同步迁移到 zod parse · 修复 Pattern D 一致性(admin 侧 scoringResult 读取默认走 runtime parse · 而非 cast)。~~ **DONE 2026-04-30**:`getAdminSessionReport` now parses hydrated `scoringResult` with `V5ScoringResultSchema.parse()` before emitting the admin report; invalid grade/shape drift fails closed before JSON output.
- ~~**shared.V5ScoringResultSchema `.strict()` tightening**(observation #128 follow-up):当前 `V5ScoringResultSchema` 是 non-strict(顶层 passthrough) · 原因是 scoring pipeline 偶发写入 transient forward-compat 字段。V5.0.5 考虑审计所有 writer · 转成 `.strict()` · 做 gate 而非 parse 轮廓。~~ **DONE 2026-04-30**:writer/reader audit found no production writer adding extra top-level `V5ScoringResult` fields; the only forward-compat case was the shared schema test's synthetic `newV5_1Field`. Schema is now top-level `.strict()` so report-facing contract growth requires an explicit shared type/schema update; nested signal and boundary-analysis payloads remain passthrough for algorithm-specific evidence details.
- ~~**scoring-hydrator cached-result parse**(observation #205 follow-up):`ScoringHydratorService` cache-hit path read `Session.scoringResult` JSON and cast it to `V5ScoringResult`, so deeper service cache semantics could bypass report-boundary schema gates.~~ **DONE 2026-04-30**:cache hits now run `V5ScoringResultSchema.parse()` before returning `cached=true`; invalid grade drift and unknown top-level strict-schema drift fail closed without re-scoring or re-persisting.
...
- ~~**CandidateApiError `details` field**:backend AppError envelope supports `{ error: { code, message, details? } }` per drift #6. Today's `CandidateApiError` only carries `{ code, status, message }` — F-A12 C2 test 4 explicitly skipped `details` assertion (T2 α ratify). When a downstream consumer needs field-level 422 breakdown (e.g. surface "which field failed"), widen `CandidateApiError` with `readonly details?: unknown` and thread it through `parseErrorBody`. Non-breaking add.~~ **DONE 2026-04-30**:`CandidateApiError` now carries `readonly details?: unknown`; `parseErrorBody` preserves nested AppError `details`; submitProfile 422 test asserts field-level details survive without changing legacy flat-envelope behavior.
...
- ~~**shared `BilingualText` interface consolidation**:four client surfaces (`consentContent.ts`, `profileContent.ts`, `selfViewContent.ts`, `transparencyContent.ts`) each locally re-declare the same `{ readonly zh: string; readonly en: string }` shape under the name `BilingualText`. `transparencyContent.ts` additionally introduces `BilingualParagraphs` (`{ readonly zh: readonly string[]; readonly en: readonly string[] }`). Hoist both types into a shared utility (e.g. `packages/client/src/lib/bilingual.ts`, or into `@codelens-v5/shared` if any server surface ever needs them) and re-export. Touches 4 import sites + 1 new module. Mechanical deduplication · no behavior change.~~ **DONE 2026-04-30**:`packages/client/src/lib/bilingual.ts` now owns `BilingualText` and `BilingualParagraphs`; Consent/Profile/SelfView/Transparency content files and page consumers import the client-local type. Chose client-local over `@codelens-v5/shared` because the type is UI copy-only and has no server contract.
- ~~**frontend-page-shape template**(observation #145):the four shipped A-series Frontend PRs (Consent F-77, ProfileSetup F-A12, SelfView F-A10-lite, Transparency A15) share a stable `{feature}Content.ts` + `{Feature}Page.tsx` + `{Feature}Page.test.tsx` shape that emerged organically without codification. Promote to `docs/v5-planning/frontend-page-shape.md` template so future candidate-facing (and public) pages inherit the pattern mechanically — co-locate bilingual copy, centralize `lib/tokens` styling, test via MemoryRouter + data-testid DOM assertions, keep handlers pure / `window.history`-safe. Doc-only task · 0 prod / 0 test.~~ **DONE 2026-04-30**:`docs/v5-planning/frontend-page-shape.md` now codifies the content/page/test triad, route-boundary guard naming, bilingual copy rules, route/auth state handling, DOM anchors, ethics-floor negative assertions, and PR cross-checks. See observation #197.
- ~~**`TransparencyStatement.tsx` audience-context rename candidate**(observation #143):with A15 shipping a public `TransparencyPage` at `/transparency`, the existing `report/sections/TransparencyStatement.tsx` (report trailer disclaimer) is easily confusable by name. Consider renaming to `ReportTrailerDisclaimer.tsx` or `ReportTransparencyFooter.tsx` to make the in-context-report-trailer scope explicit. Pure rename · ~3 file touch (component, `ReportViewPage.tsx` import, test). Non-urgent · ride with V5.0.5 signal-count unify PR.~~ **DONE 2026-04-30**:renamed live report trailer component/test/export/import to `ReportTransparencyFooter`; public `TransparencyPage` remains separate. DOM test ids and copy unchanged. See observation #195.
