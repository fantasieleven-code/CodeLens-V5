/**
 * Volcano RTC Voice Chat Service (V5).
 *
 * Manages the AI voice agent lifecycle for Module C: Start / Update / Stop.
 * Ported from V4 (archive/v4/mc/voice-chat.service.ts, 523 lines) — the
 * Volcano RTC API surface is schema-agnostic, so this file stays close to
 * the V4 implementation. The CP4_ prefix on env vars is retained as
 * vocabulary-only (scope discipline, avoids env churn).
 *
 * V5 changes:
 *   - Header + logger prefix say `[voice-chat-v5]`
 *   - Module C candidate greeting uses V5 suite vocabulary (MA / MB / MD / SE)
 *   - LLMConfig Custom data schema is produced by routes/voice.ts using V5
 *     submissions; this service just forwards the opaque JSON blob.
 *
...
const VOLC_API_VERSION = '2025-06-01';
const VOLC_REGION = 'cn-beijing';
const VOLC_SERVICE = 'rtc';

// ─── GPS meta instruction (internal-only; LLM must not verbalize self-checks) ───
...
export function buildLLMConfig(
  ...
  if (mode === 'custom') {
    return {
      ...
      APIKey: env.RTC_CALLBACK_SECRET || env.JWT_SECRET,
...
async function volcApiRequest(action: string, body: object): Promise<unknown> {
  ...
  if (err?.Code === 'InvalidTimestamp' && cachedTimeOffset === 0) {
    logger.warn(`[voice-chat-v5] InvalidTimestamp on ${action}, detecting time offset…`);
  ...
  if (!response.ok) {
    logger.error(
      `[voice-chat-v5] Volcano API ${action} failed: status=${response.status}`,
  ...
  if (resErr) {
    logger.warn(`[voice-chat-v5] Volcano API ${action} warning: ${JSON.stringify(resErr)}`);
  }
...
class VoiceChatService {
  async startVoiceChat(params: StartVoiceChatParams): Promise<{ taskId: string }> {
    ...
    const overrides =
      params.customLlmUrl || params.customData
        ? {
    ...
    logger.info(
      `[voice-chat-v5] Starting voice chat: room=${params.roomId} task=${params.taskId} mode=${mode} target=${params.targetUserId} agent=${env.VOLC_AI_USER_ID}`,
    );

    const result: Record<string, unknown> = (await volcApiRequest(
      'StartVoiceChat',
      body,
    ...
    logger.info(
      `[voice-chat-v5] StartVoiceChat response: RequestId=${reqId} room=${params.roomId} task=${params.taskId} result=${JSON.stringify(resResult || {})}`,
    );
  ...
  async updateVoiceChat(params: UpdateVoiceChatParams): Promise<void> {
    ...
    logger.info(`[voice-chat-v5] Update: room=${params.roomId} command=${params.command}`);
  ...
  async stopVoiceChat(roomId: string, taskId: string): Promise<void> {
    ...
    logger.info(`[voice-chat-v5] Stopping voice chat: room=${roomId}`);
  ...
  /** Refresh SystemMessages with updated progress via UpdateConfig. */
  async pushGpsUpdate(roomId: string, taskId: string, newSystemMessage: string): Promise<void> {
    ...
    logger.info(`[voice-chat-v5][gps] SystemMessages refreshed: room=${roomId}`);
  }

  /** Send ExternalTextToSpeech with configurable interrupt priority. */
  async speakText(roomId: string, taskId: string, text: string, interruptMode = 1): Promise<void> {
    await this.updateVoiceChat({
      roomId,
      taskId,
  ...
  /** Whether the RTC service has enough env for live calls. */
  isAvailable(): boolean {
    return !!(env.VOLC_RTC_APP_ID && env.VOLC_RTC_APP_KEY && env.VOLC_AK && env.VOLC_SK);
  }

  /**
   * Start voice chat with graceful fallback to text mode when RTC is
   * unavailable (shared credentials not set / transient failure).
   */
  async delayedStartVoiceChat(
    params: StartVoiceChatParams,
  ): Promise<{ started: boolean; taskId: string; fallbackToText: boolean }> {
    if (!this.isAvailable()) {
      logger.info('[voice-chat-v5] RTC not available, falling back to text mode');
    ...
    try {
      ...
      logger.warn('[voice-chat-v5] Failed to start RTC, falling back to text mode:', err);
      return { started: false, taskId: params.taskId, fallbackToText: true };
    }
  }
}

// ─── Time offset detection (± 15 min tolerance) ───
...
export async function findTimeOffset(): Promise<number> {
  ...
  const body = {
    ...
    Config: {
      ...
      TTSConfig: {
        Provider: 'volcano_bidirection',
        ProviderParams: { Credential: { ResourceId: 'seed-tts-1.0' } },
      },
      LLMConfig: {
        Mode: 'ArkV3',
  ...
  for (const offset of offsets) {
    try {
      ...
      if (!err) {
        ...
        if (offset !== 0) logger.info(`[voice-chat-v5] Time offset detected: ${offset}s`);
