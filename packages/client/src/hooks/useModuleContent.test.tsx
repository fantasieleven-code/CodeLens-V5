...
describe('useModuleContent', () => {
  it('transitions loading → loaded on 200', async () => {
    fetchMock().mockResolvedValueOnce({ ok: true, status: 200, json: async () => FAKE_VIEW });
    const { result } = renderHook(() => useModuleContent('e-1', 'mb'));
    expect(result.current.status).toBe('loading');
    await waitFor(() => expect(result.current.status).toBe('loaded'));
    if (result.current.status === 'loaded') expect(result.current.data).toEqual(FAKE_VIEW);
  });
  ...
  it('transitions loading → error on 404', async () => {
    ...
    if (result.current.status === 'error') expect(result.current.message).toContain('未找到');
  });
  ...
  it('transitions loading → error on network exception', async () => {
    ...
    const { result } = renderHook(() => useModuleContent('e-1', 'mb'));
    await waitFor(() => expect(result.current.status).toBe('error'));
    if (result.current.status === 'error') expect(result.current.message).toBe('网络错误');
  });
});
