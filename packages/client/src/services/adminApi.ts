/**
 * Admin API client (Task 10 mock-first).
 *
 ...
 * real implementation that is a fetch stub — Task 12 will wire it to Backend
 * Task 15. The `VITE_ADMIN_API_MOCK` env switch (or the absence of
 * `VITE_API_URL`) selects which implementation is exported.
 */
...
async function mockGetStatsOverview(): Promise<AdminStatsOverview> {
...
async function mockGetSession(sessionId: string): Promise<AdminSessionSummary> {
...
async function mockGetSuites(): Promise<AdminSuiteSummary[]> {
...
// ── Real implementations (stubs — Task 12 wires these up) ────────────
...
async function realCreateSession(
  ...
  const res = await fetch(`${requireApiUrl()}/admin/sessions/create`, {
    method: 'POST',
...
async function realListSessions(
  ...
  const res = await fetch(`${requireApiUrl()}/admin/sessions?${qs.toString()}`);
  if (!res.ok) throw new Error(`listSessions failed: ${res.status}`);
  return res.json() as Promise<PaginatedSessions>;
}

async function realGetSession(sessionId: string): Promise<AdminSessionSummary> {
  const res = await fetch(`${requireApiUrl()}/admin/sessions/${sessionId}`);
  if (!res.ok) throw new Error(`getSession failed: ${res.status}`);
  return res.json() as Promise<AdminSessionSummary>;
}
...
async function realGetSessionReport(
  ...
  const res = await fetch(
    `${requireApiUrl()}/admin/sessions/${sessionId}/report`,
  );
  if (!res.ok) throw new Error(`getSessionReport failed: ${res.status}`);
  return res.json() as Promise<AdminSessionReport>;
}
...
async function realListExamInstances(params: {
  ...
  const res = await fetch(
    `${requireApiUrl()}/admin/exam-instances?${qs.toString()}`,
  );
  if (!res.ok) throw new Error(`listExamInstances failed: ${res.status}`);
  return res.json() as Promise<AdminExamInstance[]>;
}

async function realGetSuites(): Promise<AdminSuiteSummary[]> {
  const res = await fetch(`${requireApiUrl()}/admin/suites`);
  if (!res.ok) throw new Error(`getSuites failed: ${res.status}`);
  return res.json() as Promise<AdminSuiteSummary[]>;
}

async function realGetStatsOverview(): Promise<AdminStatsOverview> {
  const res = await fetch(`${requireApiUrl()}/admin/stats/overview`);
  if (!res.ok) throw new Error(`getStatsOverview failed: ${res.status}`);
  return res.json() as Promise<AdminStatsOverview>;
}
